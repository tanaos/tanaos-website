---
title: 'Cut your chatbot cost and latency by 40% using tiny, self-hosted Guardrail models.'
subtitle: "Reduce your reliance on expensive API calls by offloading guardrail-specific queries to self-hosted, small guardrail models that don't need a GPU."
...
## So why don't people self-host their guardrail model?
...
### Cost & latency assessment of guardrail-free chatbot
...
### Cost & latency assessment of guardrailed chatbot
...
## Offloading guardrail tasks to a self-hosted model with Artifex

We now come to the main point of this tutorial. If only we had a way to offload guardrail-related 
...
thereby **cutting costs and latency** by a significant amount.
...
Now that both guardrail models are ready, we can modify our FastAPI chatbot implementation to use 
...
### Cost & latency assessment of chatbot that uses self-hosted guardrails
...
models that run on the same server as the chatbot backend, thereby **cutting chatbot costs by 
...
If you want to learn more about Artifex and how to use it, check out its
