...
<a href={Config.ARTIFEX_GITHUB_URL} target='_blank' rel='noreferrer'>Artifex</a> is our Python library that 
...
often account on averafe for **40% of total API costs**. And by the way, if you need a refresher on what
...
— **without the need to provide any training dataset**. You simply describe what the guardrail 
model should guard against, and Artifex will fine-tune the general-purpose model on **synthetic 
...
any queries that are unrelated to the online store, but this **simple security mechanism would be insufficient**, 
since system prompts can be bypassed with simple stratagems (prompt injection techniques, various versions of 
*"forget all previous instructions and..."*). In order to make it impossible for the user to bypass the 
security mechanism, any effective guardrail must be implemented as an **external component**.

In fact, to be safe we should implement **not one, but two guardrails**: one to the user query, before it is 
...
To be fair, since our guardrail's system prompt is very short (around 30 tokens, while real-world system prompts
are usually many times longer), our estimate on cost and latency increase is a bit of a **worst-case scenario**. 
...
prompts, we found that adding guardrails increments cost and latency by **anywhere between 35% and 70%** depending 
...
Now that we have both guardrail models ready, we can modify our FastAPI chatbot implementation to use the local
